...
export const PlantEditor = (props) => {
  ...
  const { plant_feature_data, inserted_plant, saving_feature } = data;
  return (
...
const SavingPlantFeature = (props) => {
  const { act, data } = useBackend();
  const { current_feature, current_feature_traits } = data;
  return (
    <Dimmer>
      <Flex direction="column">
        <Flex.Item>Choose Which Traits To Save With The Feature</Flex.Item>
...
const EmptyBody = (props) => {
  const { act, data } = useBackend();
  return <Box textAlign="center">Please Insert Plant</Box>;
};

const InUseBody = (props) => {
  const { act, data } = useBackend();
  const { plant_feature_data, inserted_plant, current_feature } = data;
  return (
    <Flex direction="row">
      <Flex.Item width="35%">
        <Section>
...
const PlantDataTab = (props) => {
  const { act, data } = useBackend();
  const { current_feature, current_feature_data, current_feature_traits } = data;
  return (
    <Flex direction="column">
...
const PlantTraitInstance = (props) => {
  ...
  const { save_excluded_traits } = data;
  return (
