...
/obj/item/food/bread/make_processable()
	if (slice_type)
		AddElement(/datum/element/processable, TOOL_KNIFE, list(slice_type = yield), 3 SECONDS, table_required = TRUE)
		AddElement(/datum/element/processable, TOOL_SAW, list(slice_type = yield), 4 SECONDS, table_required = TRUE)
