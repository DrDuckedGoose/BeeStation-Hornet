...
/datum/endo_assembly/item/eyes/New(datum/parent)
	. = ..()
...
/datum/endo_assembly/item/eyes/proc/catch_assembly(datum/source, mob/M)
	SIGNAL_HANDLER

	var/obj/item/organ/eyes/eyes = locate(/obj/item/organ/eyes) in parts
	if(!eyes)
		return
	eyes.owner = M
	eyes.Insert(M)

/datum/endo_assembly/item/eyes/proc/catch_unassembly(datum/source, mob/M)
	SIGNAL_HANDLER
...
/*
	MMI & Posibrains
	handles giving the brain control of the mob
*/
...
/*
	Radio
	Does radio shit, sets the robot's radio
*/
...
/*
	Lamp
	Gives goober robot a light to see in dark places
*/
...
/*
	Law circuits
	Gives the robot its laws, or none
*/
/datum/endo_assembly/item/law_circuit
	item_requirment = /obj/item/law_circuit
	poll_path = /obj/item/law_circuit
	allow_poll = TRUE

/datum/endo_assembly/item/law_circuit/New(datum/parent)
	. = ..()
	RegisterSignal(part_parent, COMSIG_ENDO_ASSEMBLE, PROC_REF(catch_assembly))
	RegisterSignal(part_parent.parent, COMSIG_ENDO_UNASSEMBLE, PROC_REF(catch_unassembly))

/datum/endo_assembly/item/law_circuit/proc/catch_assembly(datum/source, mob/M)
	SIGNAL_HANDLER

	var/mob/living/silicon/new_robot/R = M
	if(!istype(R))
		return
	var/obj/item/law_circuit/L = locate(/obj/item/law_circuit) in parts
	if(!L)
		return
	//handle law setup
	L.parent_robot(R)

/datum/endo_assembly/item/law_circuit/proc/catch_unassembly(datum/source, mob/M)
	SIGNAL_HANDLER
